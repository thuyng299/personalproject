...
public class RegionServiceImpl implements RegionService {
    ...
    public RegionDTO createRegion(RegionCreateDTO regionCreateDTO) {
        if (regionCreateDTO.getName() == null || regionCreateDTO.getName().isBlank()) {
            throw WarehouseException.badRequest("InvalidName", "Region name cannot be null!");
        }
        Region region = Region.builder()
                .name(regionCreateDTO.getName())
                .build();
    ...
    @Override
    public List<RegionDTO> findByIdOrName(Long regionId, String regionName) {
        List<Region> regions = regionRepository.findByIdOrName(regionId, regionName);
        if (regionName == null || regionName.trim().isBlank() || regionName.isEmpty()) {
            throw WarehouseException.badRequest("InvalidName", "Region name cannot be null");
        }
        if (regions.isEmpty()) {
